 ...
 * and RAPTOR. The admitted contract transformations remove the non-operative
 ...
 * such as REPLAY or RAPTOR fit the tokenizer better. This is intentionally
...
export async function eclipseEncode(text: string, enc: EncodingName = 'o200k_base'): Promise<EclipseResult> {
  const [zenith, splice, replay, raptor] = await Promise.all([
  ...
  return eclipseFromCandidates(text, zenith, splice, replay, raptor, enc);
}
