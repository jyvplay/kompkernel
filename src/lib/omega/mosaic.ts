...
const LANES: Lane[] = [
  ...
  },
  {
    ...
    tag: 'q', name: 'quasar-local',
    encode: (t, e) => { if (t.length < 64) return { wire: t, applied: false }; const r = quasarEncode(t, e); return { wire: r.wire, applied: r.mode !== 'identity' }; },
    decode: quasarDecode, prompt: QUASAR_SYSTEM_PROMPT,
  },
  {
    tag: 'x', name: 'plexus-local',
    encode: (t, e) => { if (t.length < 64) return { wire: t, applied: false }; const r = plexusEncode(t, e); return { wire: r.wire, applied: r.mode !== 'identity' }; },
    decode: plexusDecode, prompt: PLEXUS_SYSTEM_PROMPT,
  },
  {
  ...
  {
    tag: 'b', name: 'repair-local',
    encode: (t, e) => { const x = repairEncode(t, e); return { wire: x.wire, applied: x.applied }; },
    decode: repairDecode, prompt: REPAIR_SYSTEM_PROMPT,
  },
];
...
export function mosaicDecoderPrompt(r?: MosaicResult | null): string {
  ...
  const head = [
    ...
    '  g = SIGNET · h = HELIX · p = PULSE · a = ANAPHORA · d = local PRAXIS dictionary · s = local SIGMA schema fold · m = local MERIDIAN · q = local QUASAR · x = local PLEXUS · v = local VERITAS · o = local AXIOM · t = local TESSERA · r = local STRATA · b = local REPAIR grammar',
