...
/**
 * Contract compiler / partial evaluator.  With a concrete wire, only the
 * operations that actually occur are described.  This is an important
 * one-message optimization: unused grammar branches are not paid for in M.
 * With no wire, the complete grammar is printed for documentation/tests.
 */
export function hermesFractalDecoderPrompt(wire?: string): string {
  const body = wire ?? '<the HERMES-F wire in this message>';
  const framed = wire?.startsWith(HERMES_F_START + '\n') ?? false;
  const lines = framed ? wire!.slice((HERMES_F_START + '\n').length).split('\n') : [];
  const hasT = !framed || lines.some(line => line.startsWith('T\t'));
  const hasB = !framed || lines.some(line => line.startsWith('B\t'));
  const hasR = !framed || lines.some(line => line.startsWith('R\t'));
  const ops = new Set<string>();
  if (!framed) ['=', '#', '%', '~'].forEach(op => ops.add(op));
  for (const line of lines) {
    if (!line.startsWith('T\t')) continue;
    try {
      const specs = JSON.parse(line.split('\t')[3]) as string[];
      for (const spec of specs) if (typeof spec === 'string' && spec) ops.add(spec[0]);
    } catch { /* malformed wires get the short generic contract; decode rejects them */ }
  }
  const clauses: string[] = [];
  if (hasT) clauses.push('T id L S: JSON arrays, |L|=|S|+1; unit=L0,s0,L1,...,sk,Llast.');
  if (hasB) clauses.push('B id n t: repeat T[id] n times, join by newline, trailing newline iff t=1.');
  if (hasR) clauses.push('R J: append JSON string J.');
  if (ops.has('=')) clauses.push('=J repeats J.');
  if (ops.has('#')) clauses.push('#a,d gives a+i*d.');
  if (ops.has('%')) clauses.push('%J cycles JSON strings.');
  if (ops.has('~')) clauses.push('~J lists n JSON strings.');
  return [
    'H-F exact; no external tools/state. WIRE:',
    body,
    `If WIRE does not start with ${HERMES_F_START} plus newline, copy it. Else parse tab-lines. ${clauses.join(' ')} Ignore T; concatenate B/R; output exact text.`,
  ].join('\n');
}
