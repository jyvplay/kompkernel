 ...
 *
 * Designed to strictly dominate STARLIGHT-S1, VERITAS-VX, MOSAIC, QUASAR,
 * MERIDIAN, and all existing byte-exact direct-reasoning codecs on chaotic
...
export function astraeaEncode(text: string, enc: EncodingName = 'o200k_base'): AstraeaResult {
  ...
  while (aliasIdx < aliasPool.length && entries.length < MAX_DICTIONARY_ENTRIES) {
    ...
    for (const phrase of topCandidates) {
      ...
      const trialWire = assembleAstraeaWire([...entries, trialEntry], nextBody);
  ...
  if (entries.length === 0) {
    ...
    return {
      ...
      entries: [],
      mode: 'forced-wrap',
      notes: 'forced empty-dict wrap (input begins with ASTRAEA sentinel)',
      encodeMs: ms(),
    };
  }

  const wire = assembleAstraeaWire(entries, currentBody);
  const outTokens = countTokens(wire, enc);

  // Exactness Gate G1: Roundtrip Verification
  const decoded = astraeaDecode(wire);
  if (decoded !== text) {
  ...
  return {
    ...
    entries,
    mode: 'astraea',
    notes: `★ ASTRAEA-A1: ${entries.length} CJK single-token contractions · micro-header wire · verified byte-exact`,
    encodeMs: ms(),
  };
}
