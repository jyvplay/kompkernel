/**
 * ★ CHRONOS-X1 — Terminal Direct-Reasoning Byte-Exact Lossless Codec
 * =============================================================================
 ...
 *
 * KEY INNOVATIONS:
 * 1. Disjoint Sentinel (`★X\n`) preventing cross-codec decoder ambiguity
 * 2. Equals-Free Fast Micro-Header (`★X\n<alias><phrase>\n\n<body>`)
...
export interface ChronosResult {
  ...
  mode: 'chronos' | 'identity' | 'forced-wrap';
  notes: string;
  encodeMs: number;
}
...
export function chronosDecode(wire: string): string {
  ...
  // Equals-free micro-header format: ★X\n<alias><phrase>\n<alias><phrase>\n\nbody
  // Or empty dict wrap: ★X\n\nbody
...
function assembleChronosWire(entries: ChronosEntry[], body: string): string {
  if (entries.length === 0) {
    return SENTINEL + '\n' + body;
  }
...
export function chronosEncode(text: string, enc: EncodingName = 'o200k_base'): ChronosResult {
  ...
  const identity = (notes: string): ChronosResult => ({
    wire: text,
    decoded: text,
    exact: true,
    inTokens,
  ...
  while (aliasIdx < aliasPool.length && entries.length < MAX_DICTIONARY_ENTRIES) {
    const currentWireTokens =
      entries.length > 0
        ? countTokens(assembleChronosWire(entries, currentBody), enc)
        : inTokens;
    ...
    for (const phrase of topCandidates) {
      ...
      const trialWire = assembleChronosWire([...entries, trialEntry], nextBody);
  ...
  if (entries.length === 0) {
    ...
    return {
      ...
      entries: [],
      mode: 'forced-wrap',
      notes: 'forced empty-dict wrap (input begins with CHRONOS sentinel)',
      encodeMs: ms(),
    };
  }

  const wire = assembleChronosWire(entries, currentBody);
  const outTokens = countTokens(wire, enc);

  // Exactness Gate G1: Roundtrip Verification
  const decoded = chronosDecode(wire);
  if (decoded !== text) {
  ...
  return {
    ...
    entries,
    mode: 'chronos',
    notes: `★ CHRONOS-X1: ${entries.length} CJK single-token contractions · equals-free micro-header wire · verified byte-exact`,
    encodeMs: ms(),
  };
}

export const CHRONOS_SYSTEM_PROMPT = [
  '# ★ CHRONOS-X1 — terminal byte-exact equals-free CJK single-token contractive dictionary wire',
  ...
  '5. If there is no ★X header, the text is literal.',
...
export function chronosSelfTest(enc: EncodingName = 'o200k_base'): ChronosSelfTest[] {
  const sample900 =
  ...
  const cases: { name: string; text: string }[] = [
    { name: 'X0 empty', text: '' },
    { name: 'X1 short prose', text: 'The quick brown fox jumps over the lazy dog.' },
    { name: 'X2 900-char chaotic hetero text', text: sample900 },
    { name: 'X3 sentinel adversary', text: '★X\nfake trap\n\nnot real' },
    { name: 'X4 CRLF + unicode', text: 'line1\r\nline2\r\n中文 🚀🚀 ≈done\r\n' },
    {
      name: 'X5 repetitive JSON log',
      text: Array.from(
        { length: 30 },
        (_, i) =>
  ...
  for (const c of cases) {
    try {
      const r = chronosEncode(c.text, enc);
      const roundTrip = chronosDecode(r.wire) === c.text;
