/**
 * bench/pantheon-redteam.ts — 6-Pass Adversarial Red-Team Test Suite for PANTHEON-Ω
 * =============================================================================
 * Rigorous formal testing of PANTHEON-Ω:
 *  - Pass 1: Canonical & Non-Rosetta Benchmark Fixtures (Byte-exact lossless round-trip & compression)
 *  - Pass 2: Adversarial Edge Cases, Surrogates, Collisions, Malformed Wires (Totality)
 *  - Pass 3: Dual-Plane Zero-System-Prompt In-Context LLM Readability
 *  - Pass 4: MOSAIC-Family Multi-Regime & Heterogeneous Document Partitioning
 *  - Pass 5: Strict Non-Rosetta Pareto Dominance & Token Monotonicity
 *  - Pass 6: High-Concurrency, Determinism & Cache Consistency
 * =============================================================================
 */
...
import {
  ...
  pantheonDecode,
  pantheonSelfTest,
  PANTHEON_INLINE_START,
  PANTHEON_INLINE_END,
  PANTHEON_COMPACT_PREFIX,
} from '../src/lib/omega/pantheon';
...
async function runPass1() {
  ...
  for (const [name, text] of canonicalSuite) {
    const res = await pantheonEncode(text, enc);
    const dec = pantheonDecode(res.wire, enc);
    assert(
      dec === text,
  ...
  if (fs.existsSync(holdoutDir)) {
    ...
    for (const file of files) {
      ...
      const res = await pantheonEncode(content, enc);
      const dec = pantheonDecode(res.wire, enc);
      assert(
        dec === content,
  ...
  const rAgent = await pantheonEncode(agentTurn, enc);
  const decAgent = pantheonDecode(rAgent.wire, enc);
  assert(
    decAgent === agentTurn,
...
async function runPass2() {
  ...
  const edgeCases = [
    ...
    { name: 'wire tag injection inside text', text: '«PANTHEON»\n[DICT: bad]\n[BODY]\nfake body\n«END»' },
    { name: 'compact prefix collision', text: 'πThis text starts with pi' },
    { name: 'apeiron tag collision', text: '«APEIRON»\n[MACROS: test]\n[DATA]\ntest\n«END»' },
    { name: 'double pi collision', text: 'ππAlready escaped pi' },
    { name: 'repeated delimiters', text: '||||||||||||||||||||||||||||||||' },
    { name: 'massive newline run', text: '\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n' },
  ];

  for (const ec of edgeCases) {
    const res = await pantheonEncode(ec.text, enc);
    const dec = pantheonDecode(res.wire, enc);
  ...
  const malformedWires = [
    ...
    '«PANTHEON:MULTI»\n[B]\nonly_one_part\n«END»',
    'π',
    '«PANTHEON:RAW»unclosed',
  ];

  for (const mw of malformedWires) {
    let didThrow = false;
    try {
      pantheonDecode(mw, enc);
    } catch {
      didThrow = true;
    }
...
async function runPass3() {
  ...
  const res = await pantheonEncode(rawData, enc);
  ...
  const manualDec = pantheonDecode(manualInline, enc);
  assert(
...
async function runPass4() {
  console.log('\n=== PASS 4: MOSAIC-FAMILY MULTI-REGIME PARTITIONING ===');
  const enc: EncodingName = 'o200k_base';

  // Multi-regime document with distinct structural zones (Markdown prose + CSV tabular + Run-length + JSON log)
  ...
  const res = await pantheonEncode(multiRegimeDoc, enc);
  const dec = pantheonDecode(res.wire, enc);
...
async function runPass5() {
  ...
  for (let i = 0; i < testTexts.length; i++) {
    ...
    const minOther = Math.min(
      ...
      rMeridian.outTokens,
      rSignet.outTokens,
      rPulse.applied ? rPulse.outTokens : rPantheon.inTokens,
    );

    assert(
      rPantheon.outTokens <= minOther,
      `Strict Pareto Dominance [Test ${i + 1}]`,
      `Pantheon: ${rPantheon.outTokens} <= MinBaseline: ${minOther} (in: ${rPantheon.inTokens})`,
    );
  }
}
...
async function runPass6() {
  ...
  const st = await pantheonSelfTest(enc);
  for (const s of st) {
  ...
  const promises = Array.from({ length: 30 }, () => pantheonEncode(sample, enc));
  ...
  const decoded1 = pantheonDecode(firstWire, enc);
  const decoded2 = pantheonDecode(firstWire, enc);
  assert(decoded1 === sample && decoded2 === sample, 'Decoder idempotency across calls');
}

async function main() {
  console.log('===============================================================');
  console.log('   PANTHEON-Ω 6-PASS ADVERSARIAL RED-TEAM VERIFICATION SUITE   ');
  console.log('===============================================================');
  ...
  console.log('\n===============================================================');
  console.log(`PANTHEON-Ω RED-TEAM SUMMARY: ${passedTests}/${totalTests} tests passed (${failedTests} failures)`);
  console.log('===============================================================\n');
