...
Date: 2026-09-25 UTC. This file records the contract, construction, attacks, and executable receipts for the new codec in `src/lib/omega/hermes-fractal.ts`.
...
The reader receives `hermesFractalDecoderPrompt(wire)` and the wire in the same message. `T` lines define templates; `B` expands a template; `R` carries an exact JSON string. The prompt is the complete contract. The encoder and library decoder are not assumed available to the reader.
...
- `M_q(e,x) = countTokens(decoderPrompt(wire))`;
- `I_q(x) = countTokens(x)`.
...
- **H+ (restricted target):** achieved. HERMES-F is an exact, self-carried, variable-arity codec and the adaptive router is never worse than HERMES-Ω on measured `M`; the chat fixture strictly improves.
...
2. **HERMES-F variable-arity templates.** Artifact: `src/lib/omega/hermes-fractal.ts`. Proved portion: exact template discovery, variable unit width, constant/affine/cycle/literal generators, raw fallback, self-carried prompt. Gap: no global optimality; local candidate selection can miss a joint partition.
...
After the prompt/wire compaction patch and parser repair, the complete HERMES-F 9/9 red-team was rerun, followed by `npx tsc --noEmit` and `npm run build`. The repair inherited no trust: independent decoding, hostile cases, malformed frames, adaptive Pareto checks, fuzz, and speed were all rerun.

## J. Stop condition

The strongest verified artifact is HERMES-F plus `hermesAdaptiveEncode`, not a terminal universal codec. The next highest-information test is a larger held-out corpus of real mixed agent turns with exact output requirements, measuring `M`, parse errors by actual target LLMs, and encode latency. The next code experiment is a contract-aware dynamic program over HERMES-Ω and HERMES-F regions; it must retain the one-region identity/HERMES candidates so it cannot lose by framing alone.
