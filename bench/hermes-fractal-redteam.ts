...
import { hermesContractDecode, hermesContractEncode } from '../src/lib/omega/hermes-contract';
...
async function main() {
  ...
  await ok('HERMES-C specializes the prompt to the finished wire and counts the whole chat', () => {
    const text = Array.from({ length: 120 }, (_, i) => `id:${i} value:${100 + i}`).join('\n');
    const r = hermesContractEncode(text);
    assert.equal(r.codec, 'hermes-c');
    assert.equal(hermesContractDecode(r.wire), text);
    assert.equal(promptLiteralDecode(r.wire), text);
    assert.equal(r.oneChatTokens, countTokens(r.decoderPrompt, 'o200k_base'));
    assert.equal(r.messageTokens, r.oneChatTokens);
    assert.ok(r.operations.includes('T') && r.operations.includes('B') && r.operations.includes('#'));
    assert.ok(!r.operations.includes('=') && !r.operations.includes('%') && !r.operations.includes('~'));
    assert.ok(!r.decoderPrompt.includes('cycles JSON strings'));
    assert.ok(!r.decoderPrompt.includes('lists n JSON strings'));

    // Same wire, old all-operation contract: the difference isolates the
    // contract compiler rather than pretending the wire itself got shorter.
    const oldContract = [
      'HERMES-F exact; no external state/tool. WIRE:', r.wire,
    ...
    const oldTokens = countTokens(oldContract, 'o200k_base');
    assert.ok(r.oneChatTokens < oldTokens, `specialized ${r.oneChatTokens} < old ${oldTokens}`);
  });

  await ok('contract metadata and decoder remain total under malformed and adversarial frames', () => {
    const malformed = [
      `${HERMES_F_START}\nT\t0\t["x"]\t["?bad"]\nB\t0\t2\t0`,
      `${HERMES_F_START}\nB\t99\t1\t0`,
      `${HERMES_F_START}\nR\t{not-json}`,
    ...
    for (const wire of malformed) {
      const r = hermesContractEncode(wire);
      assert.equal(hermesContractDecode(wire), wire);
      assert.equal(r.decoderPrompt, hermesFractalDecoderPrompt(r.wire));
      assert.ok(Array.isArray(r.operations));
    }

    const good = hermesContractEncode(Array.from({ length: 40 }, (_, i) => `x=${i}`).join('\n')).wire;
    const mutations = [
      good.replace('B\t', 'B\t0\t0\t'),
      good.replace('T\t', 'T\t0\t'),
      good.replace('#', '?'),
      good + '\nX\tbad',
    ];
    for (const mutation of mutations) {
      assert.notEqual(mutation, good);
      assert.equal(hermesContractDecode(mutation), mutation, 'repair gate rejects mutated frame');
    }
  });
