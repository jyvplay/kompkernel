...
function pickSigils(text, enc) {
  const free = ideographPool2(enc).filter((ch) => text.indexOf(ch) === -1);
...
function pickSigils2(text, enc) {
  const free = ideographPool3(enc).filter((ch) => text.indexOf(ch) === -1);
  if (free.length < 5) return null;
...
function columnCandidates(vals, s, depth, enc) {
  ...
  if (n >= 2 && vals.every((v) => /^\d{1,15}$/.test(v))) {
    ...
    if (Number.isSafeInteger(start) && Number.isSafeInteger(stride)) {
      ...
      if (ok2) out.push("#" + start + "," + stride + "," + width);
    }
  }
...
function repairEncode(text, enc = "o200k_base") {
  ...
  const free = ideographPool3(enc).filter((x) => !text.includes(x));
  ...
  let bestWire = text;
  let bestTokens = inTokens;
  for (let pass2 = 0; pass2 < MAX_RULES && symbols.length > 1 && pass2 < free.length; pass2++) {
    const counts = /* @__PURE__ */ new Map();
...
function stencilEncode(text, enc = "o200k_base") {
  ...
  if (lines.length > MAX_LINES4) return identity(`STENCIL: over ${MAX_LINES4} lines \u2014 skipped for latency.`);
...
function tokenGrid2(text, enc) {
  ...
  const ok2 = pend.length === 0 && chars === text.length;
  return { ok: ok2, piece, off, idcum };
}
...
function anaphoraEncodeCore(text, enc) {
  ...
  const pool = ideographPool4(enc).filter((ch) => text.indexOf(ch) === -1);
...
function meridianEncode(text, enc = "o200k_base") {
  ...
  if (best.mode === "identity") {
    if (text.startsWith(SENTINEL6)) {
      const pool = ideographPool4(enc).filter((ch) => text.indexOf(ch) === -1);
      if (pool.length >= 2) {
        const w = SENTINEL6 + pool[0] + pool[1] + "\n" + text;
...
function cascadeInPlace(text, enc) {
  ...
  const pool = ideographPool5(enc).filter((ch) => text.indexOf(ch) === -1);
  if (pool.length < 3) return { wire: text, entries: [], mode: "identity" };
  const OPEN = pool[0];
  const CLOSE = pool[1];
  const keys = pool.slice(2);
...
function plexusEncode(text, enc = "o200k_base") {
  ...
  if (h.mode === "factored") {
    const p22 = cascadeInPlace(h.wire, enc);
    if (p22.mode === "plexus") {
      cands.push({ wire: p22.wire, mode: "compose-hp", entries: p22.entries });
    }
  }
  let best = cands[0];
  let bestTok = inTokens;
  ...
  if (best.mode === "identity") {
    if (text.startsWith(SENTINEL8)) {
      const pool = ideographPool5(enc).filter((ch) => text.indexOf(ch) === -1);
      if (pool.length >= 2) {
        const w = SENTINEL8 + pool[0] + pool[1] + "\n" + text;
...
function veritasDecode(wire) {
  ...
  const lines = wire.slice(SENTINEL9.length).split("\n");
  const dict = /* @__PURE__ */ new Map();
  let bodyStart = -1;
  for (let i = 0; i < lines.length; i++) {
    const ln = lines[i];
...
function axiomCore(text, enc, ledger) {
  const fail2 = { wire: text, entries: [], recalled: 0, bound: 0, ledgerAfter: ledger, ok: false };
  const pool = ideographPool6(enc).filter((ch) => text.indexOf(ch) === -1);
  if (pool.length < 4) return fail2;
  const OPEN = pool[0];
  const CLOSE = pool[1];
  ...
  body = bestBody;
  const finalEntries = bestEntries;
  if (finalEntries.length === 0) return fail2;
  const wire = SENTINEL10 + OPEN + CLOSE + "\n" + body;
  if (bodyDecode2(body, OPEN, CLOSE, seed) !== text) return fail2;
...
function sigmaDecode(wire) {
  ...
  while (true) {
    ...
    const { original, ok: ok2 } = decodeBlock(result.slice(si, re));
    if (!ok2) {
...
function mosaicEncodeUncached(text, enc) {
  ...
  let regionsOut = null;
  if (text.length <= MAX_CHARS && lines.length <= MAX_LINES5) {
  ...
  if (regionsOut && regionsOut.length >= 2) {
    ...
    const sep = ideographPool3(enc).find((ch) => haystack.indexOf(ch) === -1);
    if (sep) {
  ...
  if (!chosen) {
    ...
    const sep = ideographPool3(enc).find((ch) => text.indexOf(ch) === -1);
...
var CMModel = class _CMModel {
  ...
  predict() {
    ...
    this.prMix = p;
    const p12 = this.apm1.pp(p, c0);
    const p22 = this.apm2.pp(p, ((this.c4 & 255) << 2 | this.bitpos >> 1) & 1023);
    let pr = p + p12 + 2 * p22 >> 2;
    if (pr < 1) pr = 1;
...
async function omegaXiCompress(text, enc) {
  ...
  for (const c of cands) {
    ...
    try {
      ...
      ok2 = back === text;
      if (!ok2) note = "round-trip mismatch (rejected)";
    } catch (e) {
      note = e.message;
    }
    audit.push({
    ...
    if (ok2 && (!best || packed.digits.length < best.atoms || packed.digits.length === best.atoms && c.payload.length < best.payload.length)) {
  ...
  for (const line of lines) {
    ...
    if (trimmed.length >= 8) {
      ...
      if (commaIdx > 5 && commaIdx < trimmed.length - 1) {
        const p12 = trimmed.slice(0, commaIdx + 1);
        freq.set(p12, (freq.get(p12) ?? 0) + 1);
      }
      const spaceIdx = trimmed.indexOf(" ", Math.min(15, trimmed.length - 1));
      if (spaceIdx > 8) {
        const p22 = trimmed.slice(0, spaceIdx + 1);
        freq.set(p22, (freq.get(p22) ?? 0) + 1);
      }
    }
  }
...
function auroraEncode(text, enc = "o200k_base") {
  ...
  if (text.length <= MAX_CHARS2 && lines.length <= MAX_LINES6) {
    ...
    if (bestDelivered2 < INF) {
      ...
      if (segs.length >= 2) {
        ...
        const sep = ideographPool3(enc).find((ch) => hay.indexOf(ch) === -1);
        if (sep) {
    ...
    if (c === mark) {
      ...
      if (s[i + 1] === "F") {
        ...
        if (payloadEnd > 0) {
          ...
          let ok2 = keys.length >= 1 && keys.every((k) => KEY_RE.test(k));
          const rebuilt = [];
          if (ok2) {
      ...
      if (s[i + 1] === "Y" && sep !== null) {
        ...
        if (payloadEnd > 0) {
          ...
          if (sp > 0) {
            ...
            let ok2 = /^[A-Za-z_][\w-]*$/.test(name) && pairs.length >= 2;
            const rebuilt = [name + ":"];
            if (ok2) {
              for (const p of pairs) {
                const eq = p.indexOf("=");
    ...
    if (c === mark) {
      ...
      if (s[i + 1] === "F") {
        ...
        if (payloadEnd > 0) {
          const lines = s.slice(i + 2, payloadEnd).split("\n");
          const keys = (lines[0] ?? "").split(" ");
          let ok2 = keys.length >= 1 && keys.every((k) => KEY_RE2.test(k));
          const rebuilt = [];
          if (ok2) {
            for (let r = 1; r < lines.length; r++) {
              const vals = lines[r].split(" ");
...
async function rt(text, enc = "o200k_base") {
...
async function p1() {
  ...
  const shapes = [
    ...
    ["astral + zwj", "\u{1F469}\u200D\u{1F469}\u200D\u{1F467}\u200D\u{1F466} \u{1F3F3}\uFE0F\u200D\u{1F308} emoji\r\nfamilia \u{1F468}\u200D\u{1F469}\u200D\u{1F466}"],
    ...
    ["long single line", "x".repeat(5e4) + ",y"],
...
async function p2() {
  ...
  const cut = (at, len) => CHAOS_900.slice(0, at) + CHAOS_900.slice(at + len);
  const i1 = CHAOS_900.indexOf("20260915");
  const i2 = CHAOS_900.indexOf("us-east-1");
  const i3 = CHAOS_900.indexOf('{"job"');
...
async function p3() {
  ...
  const build = () => {
    ...
    for (let i = 0; i < n; i++) {
      switch (Math.floor(rnd() * 8)) {
        case 0:
          lines.push(`2026-09-1${Math.floor(rnd() * 10)}T0${Math.floor(rnd() * 9)}:1${Math.floor(rnd() * 10)}:0${Math.floor(rnd() * 10)}Z ${pick(["INFO", "WARN", "ERROR"])} svc${i} ${pick(["ok", "retry", "timeout"])}`);
          break;
        case 1:
          lines.push(`${pick(regions)} host-${i} ${Math.floor(rnd() * 99)} ${Math.floor(rnd() * 9)}`);
          break;
        case 2:
          lines.push(`{"k${i}":"v${i}","n":${Math.floor(rnd() * 999)},"ok":${rnd() > 0.5}}`);
          break;
        case 3:
          lines.push(`${pick(["csv", "svc", "app"])},${pick(regions)},${Math.floor(rnd() * 99)},${Math.floor(rnd() * 999)}`);
          break;
        case 4:
          lines.push(pick(["\u6CE8\u610F\uFF1A\u6570\u636E\u5E93\u8FC1\u79FB\u5DF2\u5B8C\u6210\u3002", "\u691C\u77E5: \u30A2\u30E9\u30FC\u30C8\u767A\u5831\u3001\u5FA9\u65E7\u3057\u307E\u3057\u305F\u3002", "\uACB0\u8BBA: \uC778\uC99D\uC11C \uB9CC\uB8CC\uAC00 \uC6D0\uC778\uC785\uB2C8\uB2E4.", "\u5907\u6CE8\uFF1A\u7F13\u5B58\u9884\u70ED\u5931\u8D25\uFF0C\u8BF7\u68C0\u67E5\u914D\u7F6E\u3002"]));
          break;
        case 5:
          lines.push(`  field_${i}: ${Math.floor(rnd() * 5e3)}`);
          break;
        case 6:
          lines.push(pick(["run(ctx):", "def f(x):", "SELECT 1;", "git push origin main", ""]));
          if (rnd() > 0.7) lines.push("    indented content here");
          break;
        default:
          lines.push(pick(["Status: deploy finished, pods restarting.", "Queue depth climbed during the window.", "Next steps? Audit and rerun.", "Watch the retry budget closely."]));
      }
    }
    return lines.join("\n");
  };
  ...
  console.log(`  never-worse: ${neverWorse}/60 \xB7 \u2264signet: ${beatsSignet}/60 \xB7 \u2264mosaic: ${beatsMosaic}/60`);
...
async function p4() {
  ...
  const garbage = ["\u3041", "\u3041\n", "\u3041J", "\u3041C", "\u30412026", "[SG1]", "\u27D0", "\0", "\u3041" + String.fromCodePoint(1114111)];
  let noThrow = true;
...
async function main2() {
  ...
  await p9();
  console.log(`
RED-TEAM: ${pass} pass / ${fail} fail (${((Date.now() - t0) / 1e3).toFixed(1)}s)`);
