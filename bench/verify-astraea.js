...
function anaphoraEncode(text, enc2 = "o200k_base") {
  ...
  const pool = ideographPool(enc2).filter((ch) => text.indexOf(ch) === -1);
...
function bestAttemptAt(lines, i, s, enc2) {
  let best = null;
...
function transpose(text, s, enc2) {
  ...
  while (i < lines.length) {
    const attempt = bestAttemptAt(lines, i, s, enc2);
    if (!attempt) {
      pieces.push(lines[i]);
      i++;
      continue;
...
function tesseraEncode(text, enc2 = "o200k_base") {
  ...
  for (const cand of combos) {
    const t = countTokens(cand.wire, enc2);
...
function bestAttemptAt2(lines, i, s, enc2) {
  let best = null;
...
function stratify(text, s, enc2) {
  ...
  let carried;
  while (i < lines.length) {
    const here = carried !== void 0 ? carried : bestAttemptAt2(lines, i, s, enc2);
    carried = void 0;
    if (!here) {
      pieces.push(lines[i]);
      i++;
    ...
    const next = bestAttemptAt2(lines, i + 1, s, enc2);
...
function bestAttemptAt3(lines, i, s, enc2) {
  let best = null;
...
function align(text, s, enc2) {
  ...
  let carried;
  while (i < lines.length) {
    const here = carried !== void 0 ? carried : bestAttemptAt3(lines, i, s, enc2);
    carried = void 0;
    if (!here) {
      pieces.push(lines[i]);
      i++;
    ...
    const next = bestAttemptAt3(lines, i + 1, s, enc2);
    if (next && next.gain > here.gain) {
      pieces.push(lines[i]);
      i++;
      carried = next;
...
function splitWs(tok2) {
  const m = tok2.match(/^(\s*)(.*)$/s);
  return { ws: m?.[1] ?? "", core: m?.[2] ?? tok2 };
}
...
`, enc2);
  ...
  const body = toks.map((tok2, i) => {
    const rep = replaceAt.get(i);
    if (!rep) return tok2;
    const { ws } = splitWs(tok2);
    return ws + rep;
  }).join("");
...
function anaphoraEncodeCore(text, enc2) {
  ...
  const pool = ideographPool4(enc2).filter((ch) => text.indexOf(ch) === -1);
  ...
  const minePass = (src) => {
    const grid = tokenGrid2(src, enc2);
    const cands = [];
    const seen = /* @__PURE__ */ new Set();
    const push = (phrase, win, hits) => {
...
function meridianEncode(text, enc2 = "o200k_base") {
  ...
  for (const c of cands) {
    ...
    const tok2 = countTokens(c.wire, enc2);
    if (tok2 < bestTok || c.mode === "forced-wrap" && best.mode === "identity" && dec === text) {
      bestTok = tok2;
      best = c;
    }
  }
...
function quasarEncode(text, enc2 = "o200k_base") {
  ...
  const pool = buildPool(enc2, inputChars);
...
function cascadeInPlace(text, enc2) {
  ...
  const pool = ideographPool5(enc2).filter((ch) => text.indexOf(ch) === -1);
  if (pool.length < 3) return { wire: text, entries: [], mode: "identity" };
  const OPEN = pool[0];
  const CLOSE = pool[1];
  const keys = pool.slice(2);
...
function plexusEncode(text, enc2 = "o200k_base") {
  ...
  for (const c of cands) {
    ...
    if (dec !== text) continue;
    const tok2 = countTokens(c.wire, enc2);
    if (tok2 < bestTok || c.mode === "forced-wrap" && best.mode === "identity") {
      bestTok = tok2;
      best = c;
    }
  }
...
function axiomCore(text, enc2, ledger) {
  ...
  const pool = ideographPool6(enc2).filter((ch) => text.indexOf(ch) === -1);
  ...
  const D = countTokens(OPEN + (freeKeys[0] ?? pool[2]) + CLOSE, enc2);
  const A = countTokens(freeKeys[0] ?? pool[2], enc2);
  let body = text;
  const entries = [];
  const seed = /* @__PURE__ */ new Map();
  let recalled = 0;
...
function harvestLedger(text, enc2, ledger, extra) {
  ...
  const pool = ideographPool6(enc2);
...
function praxisEncode(text, enc2 = "o200k_base") {
  ...
  for (const phrase of candidates(text, enc2)) {
    ...
    const tok2 = countTokens(wire, enc2);
    if (tok2 >= bestTokens) continue;
    body = nextBody;
...
function phraseEncode(text, enc2 = "o200k_base") {
  ...
  const folded = phraseFold(text, enc2);
...
function tauDecode(wire, enc2 = "o200k_base") {
  ...
  const { mark, sep } = tauMarkers(enc2);
...
function tauEncode(text, enc2 = "o200k_base") {
  ...
  while (i < lines.length) {
    ...
    if (j - i >= 2) {
      ...
      if (s !== null && (!measure || countTokens(s.span, enc2) < countTokens(lines.slice(i, j).join("\n"), enc2))) {
        out.push(s.span);
        systems.add("P");
        i = j;
        continue;
    ...
    if (lines[i] === "```yaml") {
      ...
      if (end > 0) {
        ...
        if (s !== null) {
          const wrapped = "```yaml\n" + s.span + "\n```";
          if (!measure || countTokens(wrapped, enc2) < countTokens(lines.slice(i, end + 1).join("\n"), enc2)) {
...
function candidates2(text, enc2) {
  ...
  const push = (phrase) => {
    ...
    if (hits < 2) return;
    const tok2 = countTokens(phrase, enc2);
    const score = (tok2 - 1) * hits - tok2 - 4;
...
function atlasEncode(text, enc2 = "o200k_base") {
  ...
  try {
    const m = mosaicEncode(text, enc2);
  ...
  for (const m of members) {
    ...
    const wireTok = countTokens(m.wire, enc2);
    const delivered = wireTok + countTokens(m.contract, enc2);
...
function auroraEncode(text, enc2 = "o200k_base") {
  ...
  if (text.length <= MAX_CHARS2 && lines.length <= MAX_LINES6) {
    ...
    for (let m = 0; m < 16; m++) {
      const delivered = dp[B][m] + contractCost(m, enc2);
...
async function crownEncodeFromMembers(text, enc2 = "o200k_base", supplied = {}) {
  ...
  for (const m of members) {
    ...
    const wire = countTokens(m.wire, enc2);
    const contract = countTokens(m.contract, enc2);
...
function rosettaTranspose(text, enc2 = "o200k_base", folded = null) {
  ...
  for (let li = 0; li < lines.length; li++) {
      ...
      if (line === "```yaml") {
        ...
        if (end > 0) {
          ...
          if (ys !== null) {
            ...
            const wrapped = "```yaml\n" + span + "\n```";
            const profitable = !measure || countTokens(wrapped, enc2) < countTokens(lines.slice(li, end + 1).join("\n"), enc2);
    ...
    const tsLine = tsTransposeLine(line, mark, enc2, measure);
...
function tsTransposeLine(line, mark, enc2, measure) {
  TS_EXT.lastIndex = 0;
  let out = "";
  let last = 0;
  let m;
  while ((m = TS_EXT.exec(line)) !== null) {
    ...
    if (measure && countTokens(basic, enc2) >= countTokens(m[0], enc2)) continue;
  ...
  if (wire.length >= 2 && wire[1] === "\n") {
    ...
    if (idx >= 0) {
      ...
      if (flag !== void 0 && wire.length >= 4 && wire[2] === flag && wire[3] === "\n") {
        return expandBody(wire.slice(4), mark, regionByGlyph, phraseCodebook(enc2).byGlyph, ysep);
      }
...
async function rosettaEncodeUncached(text, enc2, supplied) {
  ...
  if (tr.wire !== null && rosettaDecode(tr.wire, enc2) === text) {
    ...
    const k = pickWindow(text, enc2);
    if (k !== null) {
      const wrapWire = rosettaPool(enc2)[k] + "\n" + text;
  ...
  if (!hasCodebookGlyph(text, enc2)) {
    const folded = phraseFold(text, enc2);
    if (folded !== text) {
      const trW = rosettaTranspose(text, enc2, folded);
...
function extractDynamicEntries(text, enc2, mark, staticGlyphCount) {
...
function astraeaTranspose(text, enc2 = "o200k_base", folded = null, useDynamic = true) {
  const empty = { wire: null, mark: "", systems: [] };
  if (!text) return empty;
  const workingText = folded ?? text;
  const k = pickWindowAstraea(workingText, enc2);
  if (k === null) return empty;
  const pool = rosettaPool(enc2);
  const mark = pool[k];
  const flagAstraeaW = pool[k + 3 + RNS1_REGIONS.length];
  const sepY = pool[k + 2 + RNS1_REGIONS.length] ?? null;
  const lexiconBook = astraeaLexiconCodebook(enc2);
  ...
  if (useDynamic) {
    const dynEntries = extractDynamicEntries(currentText, enc2, mark, lexiconBook.byGlyph.size);
...
async function astraeaEncodeUncached(text, enc2, supplied) {
  ...
  if (!hasAstraeaLexiconGlyph(text, enc2)) {
    const folded = astraeaLexiconFold(text, enc2);
    const trDW = astraeaTranspose(text, enc2, folded, true);
    if (trDW.wire !== null && astraeaDecode(trDW.wire, enc2) === text) {
      admit("astraea-DW", trDW.wire, () => astraeaDecode(trDW.wire, enc2), trDW.systems);
    }
    const trW = astraeaTranspose(text, enc2, folded, false);
...
// bench/verify-astraea.ts
var enc = "o200k_base";
var tok = (s) => countTokens(s, enc);
var CHAOS_B2 = [
  "Summary: the ingestion pipeline dropped 3 events during the failover window.",
  "- consumer lag 2.4k messages, resolved in 90s",
  "- dead-letter queue gained 12 entries (poison payloads)",
...
async function compare(name, text) {
  ...
  console.log(
    `${name.padEnd(14)} in=${String(inTok).padStart(5)}  ASTRAEA=${String(a.outTokens).padStart(4)} (${a.member.padEnd(10)}) | ROSETTA=${String(r.outTokens).padStart(4)} (${r.member.padEnd(9)}) | bestOther=${bestOtherDirect}  ${superior ? a.outTokens < r.outTokens ? "STRICT WIN \u2713" : "TIE / NEVER-WORSE \u2713" : "REGRESSION \u2717"}`
  );
...
async function main2() {
  ...
  all = await compare("chaos-B", CHAOS_B2) && all;
  all = await compare("chaos-C", CHAOS_C2) && all;
