...
async function nexusEncode(text, enc2 = "o200k_base") {
  const t0 = performance.now();
  const inTokens = countTokens(text, enc2);
  const inChars = text.length;
  const identity = (notes) => ({
  ...
  const ltp = ltpProject(text, enc2);
  ...
  const sigma = sigmaEncode(afterLtp, enc2);
  ...
  const prom = await compressPrometheusICDM(afterSigma, enc2);
...
async function compare(name, text) {
  ...
  const bestDirect = Math.min(eid, ltp, signet, mosaic, orbit, apex, nexus, splice, kappa, phrase, tau, inTok);
  ...
  console.log(
    `${name.padEnd(14)} in=${String(inTok).padStart(5)}  ROSETTA=${String(r.outTokens).padStart(4)} (${r.member})  | bestDirect=${bestDirect} (eidolon=${eid} ltp=${ltp} orbit=${orbit} signet/mosaic/splice=${signet} phrase=${phrase} tau=${tau})  ${superior ? "IN-CLASS STRICTLY-SUPERIOR \u2713" : "NOT SUPERIOR \u2717"}  \xB7 ${vsXi}`
  );
  return superior;
}
async function regimeCheck(name, text) {
  const inTok = tok(text);
  const r = await rosettaEncode(text, enc);
  const rt = rosettaDecode(r.wire, enc) === text;
  const mosaic = mosaicEncode(text, enc).outTokens;
