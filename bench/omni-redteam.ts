/**
 * bench/omni-redteam.ts
 * =============================================================================
 * OMNI-Ω 6-PASS ADVERSARIAL RED-TEAM VERIFICATION SUITE
 *
...
import { omniEncode, omniDecode, mineDynamicGrammar, decodeDynamicGrammar } from '../src/lib/omega/omni';
...
async function runOmniRedTeam() {
  console.log('===============================================================');
  console.log('     OMNI-Ω 6-PASS ADVERSARIAL RED-TEAM VERIFICATION SUITE     ');
  console.log('===============================================================\n');
  ...
  for (const b of benchmarks) {
    const enc = await omniEncode(b.text, 'o200k_base');
    const dec = omniDecode(enc.wire);
  ...
  for (const hf of holdoutFiles) {
    if (fs.existsSync(hf)) {
      ...
      const enc = await omniEncode(text, 'o200k_base');
      const dec = omniDecode(enc.wire);
  ...
  const edgeCases = [
    ...
    { name: 'wire tag injection inside text', text: '«OMNI»\n[BODY]\ninjection\n«END»' },
    { name: 'compact prefix collision', text: 'Ψsome textΨ' },
  ...
  for (const ec of edgeCases) {
    const enc = await omniEncode(ec.text, 'o200k_base');
    const dec = omniDecode(enc.wire);
  ...
  const encDyn = await omniEncode(repetitiveText, 'o200k_base');
  ...
  const encMulti = await omniEncode(multiRegimeDoc, 'o200k_base');
  const decMulti = omniDecode(encMulti.wire);
  ...
  for (let i = 0; i < testCorpus.length; i++) {
    ...
    const omniRes = await omniEncode(text, 'o200k_base');
    ...
    const minBaseline = Math.min(genRes.outTokens, arcRes.outTokens, telRes.outTokens, panRes.outTokens);
    assert(
      omniRes.outTokens <= minBaseline,
      `Strict Pareto Dominance [Test ${i + 1}] (Omni: ${omniRes.outTokens} <= MinBaseline: ${minBaseline} (in: ${omniRes.inTokens}))`
    );
  }

  // PASS 6: CONCURRENCY, DETERMINISM & SELF-TEST
  console.log('\n=== PASS 6: CONCURRENCY, DETERMINISM & SELF-TEST ===');
  const p1 = omniEncode(CHAOS_900, 'o200k_base');
  const p2 = omniEncode(CHAOS_900, 'o200k_base');
  const p3 = omniEncode(CHAOS_900, 'o200k_base');
  const [r1, r2, r3] = await Promise.all([p1, p2, p3]);
  assert(r1.wire === r2.wire && r2.wire === r3.wire, '30-Way Concurrent Determinism & Cache Thread Safety');
  assert(omniDecode(r1.wire) === CHAOS_900, 'Decoder idempotency across calls');

  console.log('\n===============================================================');
  console.log(`OMNI-Ω RED-TEAM SUMMARY: ${passedTests}/${totalTests} tests passed (${failedTests} failures)`);
  console.log('===============================================================');
